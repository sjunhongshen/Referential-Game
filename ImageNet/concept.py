...
if __name__ == '__main__':
	def visualize():
		...
		images = np.load('ImageNet_images.npy')
		print(images.shape)
		attr = np.load('ImageNet_attributes.npy')
		print(attr.shape)
		# for i in np.random.randint(0, 6762, size=20):
		for i in range(6762 - 11, 6762):
			print(list(map(codemap, np.where(attr[i])[0])))
	...
	game_config.feat_dir = '/home/Datasets/ImageNet'
	game_config.save_dir = None
	game_config.generated_dataset = True
	game_config.generated_dataset_path = '/home/Datasets/ImageNet/ImageNet_%ddis_Train.npy' % game_config.num_distractors
