...
def main():
	...
	#np.random.seed(425)
	continue_steps = 0
	restored = restore_ckpt(student, teacher, os.path.join(train_config.save_dir, 'TOM_CL_phase1'))
	...
	continue_steps = 0
	restored = restore_ckpt(student, teacher, os.path.join(train_config.save_dir, 'TOM_CL_phase2'))
	if not restored or (continue_steps > 0):
		if restored:
	...
	continue_steps = 4e4
	restored = restore_ckpt(student, teacher, os.path.join(train_config.save_dir, 'TOM_CL_phase3_5000'))
	if not restored or (continue_steps > 0):
		if restored:
		...
		while global_step < train_config.iteration_phase3:
			...
			if global_step <= 2 * train_config.iteration_phase3 / 3:
				stu_loss_pg = student.update_net(trajectory_batch, 1)
				print('\t stu pg loss: %f' % stu_loss_pg)
			else:
